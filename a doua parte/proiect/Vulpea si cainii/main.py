        ...
        if self.turn == 0:
            ...
            for dog in self.dogs_pos:
                for move in [(1, 1), (-1, 1)]:
                    ...
                    if min(new_x, new_y) < 0 or max(new_x, new_y) > 7 or (new_x, new_y) in self.dogs_pos or (
                    new_x, new_y) == self.fox_pos:
                        continue
    ...
    while True:
        ...
        if step:
            ...
            if state.is_final():
                ...
                draw_table(state, highlighted_piece, turn + " to move")
...
def main():
    ...
    pygame.display.set_caption("Iojica Mattia 244 - Vulpea si cainii")
